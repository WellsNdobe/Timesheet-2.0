...
export const inviteMember = (id: string, email: string, role: "manager" | "member", token: string) => apiRequest<{ invitation: { id: string; email: string; role: string; token: string; expiresAt: string } }>(`/api/workspaces/${id}/invitations`, token, json("POST", { email, role }));
