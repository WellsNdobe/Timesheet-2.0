...
export default function App() {
  ...
  useEffect(() => {
    const pathname = path.split("?")[0]; if (session !== undefined || pathname in publicAuthModes) return;
  ...
  const publicMode = publicAuthModes[pathname as keyof typeof publicAuthModes];
  if (publicMode || session === null) return <AuthExperience mode={publicMode ?? "login"} onAuthenticated={authenticated} />;
