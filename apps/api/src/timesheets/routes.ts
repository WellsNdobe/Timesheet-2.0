...
timesheetRouter.get("/workspaces/:workspaceId/approvals", asyncHandler(async (request, response) => {
  ...
  response.json({ approvals: await approvalSummaries(workspaceId, actor) });
}));
...
timesheetRouter.get("/workspaces/:workspaceId/approval-items/:approvalItemId", asyncHandler(async (request, response) => {
  ...
  const result = await db.transaction(async (transaction) => { const row = await approvalItemFor(transaction, workspaceId, approvalItemId); const revisions = (await transaction.select().from(timesheetApprovalRevisions).where(eq(timesheetApprovalRevisions.approvalItemId, approvalItemId)).orderBy(desc(timesheetApprovalRevisions.revisionNumber))); const latest = revisions[0]; if (!latest || (actor.role === "manager" && latest.assignedApproverMembershipId !== actor.id)) throw notFound(); return { row, revisions, entries: await transaction.select().from(timesheetReviewEntrySnapshots).where(inArray(timesheetReviewEntrySnapshots.revisionId, revisions.map((revision) => revision.id))).orderBy(asc(timesheetReviewEntrySnapshots.workDate), asc(timesheetReviewEntrySnapshots.createdAt)), events: await transaction.select().from(timesheetReviewEvents).where(inArray(timesheetReviewEvents.revisionId, revisions.map((revision) => revision.id))).orderBy(asc(timesheetReviewEvents.createdAt), asc(timesheetReviewEvents.id)) }; });
  response.json({ approval: { id: String(result.row.item.id), weekStart: result.row.sheet.weekStart, submitterMembershipId: String(result.row.sheet.membershipId), project: { id: String(result.row.item.projectId), name: result.revisions[0].projectName }, latestRevision: publicRevision(result.revisions[0]), revisions: result.revisions.map((revision) => ({ ...publicRevision(revision), entries: result.entries.filter((entry) => entry.revisionId === revision.id).map((entry) => ({ ...entry, id: String(entry.id), revisionId: String(entry.revisionId), sourceEntryId: String(entry.sourceEntryId), taskId: entry.taskId == null ? null : String(entry.taskId), startedAt: entry.startedAt?.toISOString() ?? null, endedAt: entry.endedAt?.toISOString() ?? null, createdAt: entry.createdAt.toISOString() })), events: result.events.filter((event) => event.revisionId === revision.id).map((event) => ({ id: String(event.id), revisionId: String(event.revisionId), actorMembershipId: event.actorMembershipId == null ? null : String(event.actorMembershipId), type: event.type, comment: event.comment, ...(actor.role === "admin" ? { internalReason: event.internalReason } : {}), previousApproverMembershipId: event.previousApproverMembershipId == null ? null : String(event.previousApproverMembershipId), nextApproverMembershipId: event.nextApproverMembershipId == null ? null : String(event.nextApproverMembershipId), createdAt: event.createdAt.toISOString() })) })) } });
}));
...
timesheetRouter.post("/workspaces/:workspaceId/approval-items/:approvalItemId/revisions/:revisionId/transfer", asyncHandler(async (request, response) => {
  ...
  const result = await db.transaction(async (transaction) => {
    ...
    await transaction.insert(timesheetReviewEvents).values({ revisionId: revision.id, actorMembershipId: actor.id, type: "transferred", internalReason: input.reason, previousApproverMembershipId: revision.assignedApproverMembershipId, nextApproverMembershipId: nextApprover.id });
